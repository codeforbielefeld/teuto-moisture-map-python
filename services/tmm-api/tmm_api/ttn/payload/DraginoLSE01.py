...
class DraginoLSE01(PayloadParser):
    def parse_payload(self, payload: str) -> dict:
        ...
        conductivity_value = payload["uplink_message"]["decoded_payload"][
            "conduct_SOIL"
        ]
        ...
        retval["conductivity"] = float(
            self.extract_numbers_from_string(conductivity_value)[0]
        )
        ...
        retval["conductivity_unit"] = str(
            self.extract_units_from_string(conductivity_value)[0]
        )
