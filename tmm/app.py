...
def create_point_with_common_attributes(
    name,
    device_id,
    device_brand,
    device_model,
...
def writeJsonToDb(json: dict):
    ...
    # Extract sensor data
    payload_parser = PayloadParserFactory.get_parser_for_model(
        brand=device_brand, model=device_model
    )
    ...
    project_root_path = dirname(abspath(__file__))
    influx_config_file_path = f"{project_root_path}/config.ini"

    with InfluxDBClient.from_config_file(config_file=influx_config_file_path) as client:

        write_api = client.write_api(write_options=SYNCHRONOUS)

        bat_point = (
            create_point_with_common_attributes(
                "battery",
                device_id,
                device_brand,
                device_model,
